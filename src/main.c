...
int main(void)
{
    ...
    while (!WindowShouldClose())  // Detect window close button or ESC key
    {
        ...
        if (game_state) {
            ...
            /** Update snake and snake food if needed. If game state is false that means game over
             * because collision has happened  */
        ...
        DrawText((const char *)&prv_current_score, 25, 25, 50, WHITE);
        EndDrawing();
        //----------------------------------------------------------------------------------
    }
...
static void prv_snake_draw(void)
{
    ...
    DrawRectangle(snake_elem[0].x_pos * BLOCK_SIZE, snake_elem[0].y_pos * BLOCK_SIZE, BLOCK_SIZE,
                  BLOCK_SIZE, GRAY);
    for (int i = 1; i < snake_get_len(); i++) {
        DrawRectangle(snake_elem[i].x_pos * BLOCK_SIZE, snake_elem[i].y_pos * BLOCK_SIZE,
                      BLOCK_SIZE, BLOCK_SIZE, GRAY);
    }
}

static void prv_snake_food_draw(void)
{
    DrawRectangle(snake_food_get_x_pos() * BLOCK_SIZE, snake_food_get_y_pos() * BLOCK_SIZE,
